...
TEST_CASE("Return number of microseconds since arduino board began", "[micros]") {
    ...
    REQUIRE(totalDurationBegun > 1000000);
    REQUIRE(durationCheck > 1000000us);
}
...
TEST_CASE("Checks if a character is a digit"
          "[isDigit]") {
    REQUIRE(isDigit('1'));
    REQUIRE(!isDigit('a'));
    REQUIRE(!isDigit('#'));
...
TEST_CASE("Checks if it returns a random number within a given range"
          "[isRandom]") {
