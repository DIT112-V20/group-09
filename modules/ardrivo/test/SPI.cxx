...
void slave_worker(const void* input, void* output, size_t stop, BlockingBus<DynaBufferBus>& bus_buffer, std::mutex& bus_mutex) {
    ...
    do {
        std::scoped_lock tx_lock{bus_mutex, bus_buffer.tx_mutex};
        ...
        std::scoped_lock rx_lock{bus_buffer.rx_mutex};
...
void transfer_test(std::size_t bus_no, std::size_t slave_no) {
    ...
    auto& bus_mut = board_data->spi_buses[bus_no].slaves_mut;
    ...
    SECTION("Single byte transfer"){
        ...
        auto slave_thread = std::thread(slave_worker, &slave_in, &slave_out, sizeof(slave_in), std::ref(bus_buffer), std::ref(bus_mut));
    ...
    SECTION("Multibyte transfer") {
        ...
        auto slave_thread = std::thread(slave_worker, slave_in.data(), slave_out.data(), slave_in.size(), std::ref(bus_buffer), std::ref(bus_mut));
