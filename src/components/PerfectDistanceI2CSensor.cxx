...
PerfectDistanceI2CSensor::PerfectDistanceI2CSensor(BoardData& bd, Urho3D::Node* node, const rapidjson::Value& pin) : LaserCaster{node}, ticker{} {
    ...
    bus->slaves[store.address].second.emplace<1>([&](std::size_t size) mutable {
      ...
      case (MEASUREMENT_RESULT + 10): {
          uint16_t dist = measure() * 100; // in millimeters
    ...
    ticker = RDev::make_ticker(vlx, store, *bus, *this);
}
